...
export default function MenuBuilderPage() {
    ...
    const addMegaColumn = () => {
        setMegaMenuData([...megaMenuData, { title: 'New Column', links: [] }]);
    };
    const updateMegaColumn = (idx: number, field: string, value: any) => {
    ...
    const addMegaLink = (colIdx: number) => {
        const newData = [...megaMenuData];
        newData[colIdx].links.push({ label: 'New Link', url: '/' });
        setMegaMenuData(newData);
    };
    const updateMegaLink = (colIdx: number, linkIdx: number, field: string, value: string) => {
        const newData = [...megaMenuData];
        (newData[colIdx].links[linkIdx] as any)[field] = value;
        setMegaMenuData(newData);
    };
    ...
    const removeMegaLink = (colIdx: number, linkIdx: number) => {
        const newData = [...megaMenuData];
        newData[colIdx].links = newData[colIdx].links.filter((_, i) => i !== linkIdx);
        setMegaMenuData(newData);
    };
    ...
    return (
        <div className="max-w-6xl mx-auto pb-20">
            ...
            {/* Tabs */}
            <div className="flex bg-gray-100 dark:bg-gray-800/50 p-1.5 rounded-xl mb-6 w-full max-w-md">
                {(['main', 'footer', 'mobile'] as const).map(loc => (
                    <button
                        key={loc}
                        onClick={() => setActiveLocation(loc)}
                        className={`flex-1 py-2 px-4 rounded-lg text-sm font-bold capitalize transition-all ${activeLocation === loc ? 'bg-white dark:bg-dark-surface shadow text-primary' : 'text-gray-500 hover:text-gray-800 dark:hover:text-gray-200'}`}
                    >
                        {loc} Menu
                    </button>
                ))}
                                            ...
                                            <div className="space-y-6 bg-gray-50 dark:bg-gray-800/30 p-6 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">
                                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                                    {megaMenuData.map((col, cIdx) => (
                                                        <div key={cIdx} className="bg-white dark:bg-dark-surface p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                                                            <div className="flex justify-between items-center mb-3">
                                                                <input
                                                                    type="text"
                                                                    value={col.title}
                                                                    onChange={(e) => updateMegaColumn(cIdx, 'title', e.target.value)}
                                                            ...
                                                            <div className="mb-4">
                                                                <ImagePicker
                                                                    value={col.image || ''}
                                                                    onChange={(url) => updateMegaColumn(cIdx, 'image', url)}
                                                                />
                                                            </div>
                                                            <div className="space-y-2 mb-3">
                                                                {col.links.map((link, lIdx) => (
                                                                    <div key={lIdx} className="flex gap-2">
                                                                        <input
                                                                            type="text" value={link.label} onChange={(e) => updateMegaLink(cIdx, lIdx, 'label', e.target.value)}
                                                                            placeholder="Label" className="w-1/2 text-xs p-1.5 border rounded"
                                                                        />
                                                                        <input
                                                                            type="text" value={link.url} onChange={(e) => updateMegaLink(cIdx, lIdx, 'url', e.target.value)}
                                                                            placeholder="URL" className="w-1/2 text-xs p-1.5 border rounded font-mono"
                                                                        />
                                                                        <button type="button" onClick={() => removeMegaLink(cIdx, lIdx)} className="text-red-400 hover:text-red-600"><X size={14} /></button>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                            <button
                                                                type="button"
                                                                onClick={() => addMegaLink(cIdx)}
                                                                className="w-full py-1.5 border border-dashed border-primary/50 text-primary rounded text-xs font-bold hover:bg-primary/5"
                                                            >
                                                                + Add Link
                                                            </button>
                                                        </div>
                                                ...
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded-lg text-sm font-bold hover:bg-gray-300 transition w-full"
                                                >
                                                    + Add Column Stack
                                                </button>
                                            </div>
