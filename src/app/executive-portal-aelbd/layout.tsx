...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        if (status === 'unauthenticated' && pathname !== '/executive-login') {
    ...
    // We removed the full-page loading spinner to prevent hanging on slow sessions.
    // status === 'loading' is now handled by sub-components or simply ignored until resolved.

    if (status === 'unauthenticated' || pathname === '/executive-login') {
        return <>{children}</>;
    }

    const breadcrumbs = pathname.split('/').filter(p => p && p !== 'executive-portal-aelbd');
                ...
                <header className="bg-white/80 dark:bg-dark-surface/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 z-30 flex-shrink-0">
                    <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="hidden sm:flex items-center text-sm font-medium text-gray-500 dark:text-gray-400 truncate">
                            <Link href="/executive-portal-aelbd" className="hover:text-primary transition-colors">Dashboard</Link>
                        ...
                        <div className="relative ml-2">
                            ...
                            {isUserMenuOpen && (
                                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-dark-surface rounded-xl shadow-xl border border-gray-100 dark:border-gray-800 py-2 animate-in fade-in slide-in-from-top-2 origin-top-right">
                                    ...
                                    <Link href="/executive-portal-aelbd/settings" className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                                        <Settings className="w-4 h-4" /> Profile Settings
                                    </Link>
                                    <button onClick={() => signOut({ callbackUrl: '/executive-login' })} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors">
